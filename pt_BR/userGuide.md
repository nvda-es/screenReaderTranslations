...
+ A janela NVDA Launcher — Lançador do NVDA — aparece com o contrato de licença.
Pressione ``seta para baixo`` para ler o contrato de licença, se desejar.
+ Pressione ``tab`` para mover-se para a caixa de seleção "Concordo", então pressione a ``barra de espaço`` para marcá-la.
+ Pressione ``tab`` para percorrer as opções, depois pressione ``enter`` na opção desejada.
+
...
+ No lançador — instalador —, verifique se a caixa de seleção para concordar com a licença está marcada.
+ ``Tab`` e ative o botão "Instalar NVDA neste computador".
+ Em seguida, estão as opções para usar o NVDA durante as credenciais — login — do Windows e para criar um atalho na área de trabalho.
...
+ Um diálogo "Controle de conta de usuário (UAC)" do Windows aparece perguntando "Deseja permitir que este aplicativo faça alterações em seu PC?".
+ Pressione ``alt+y`` para aceitar o prompt do UAC.
+ Uma barra de progresso é preenchida à medida que o NVDA é instalado.
...
+ Um diálogo aparece confirmando que a instalação do NVDA foi bem-sucedida.
...
+ Pressione ``tab`` para mover-se para "Usar ``capsLock`` como uma tecla modificadora do NVDA".
...
+ Use ``tab`` e ``barra de espaço`` para ajustar as outras opções nesta tela.
Estas definem se o NVDA inicia automaticamente.
+ Pressione ``enter`` para fechar o diálogo com o NVDA em execução.
+
...
+ Abra a caixa de diálogo de definir comandos — gestos de entrada — no modo de navegação.
+ Encontre o item "vai à próxima guia" na seção Modo de navegação.
+ Atribua tecla ``y`` para comando — gesto — encontrado.
+ Encontre o item "volta à guia anterior".
+ Atribua ``shift+y`` para comando encontrado.
+
...
+ mova o cursor de exploração para o conteúdo matemático.
...
+ Em seguida, ative o comando seguinte:
+
...
+ Acesse as opções de seleção como descrito acima.
+ Escolha Editar Nota.
+
...
%kc:setting
==== Abrir caixa de diálogo Selecionar Sintetizador ====
...
%kc:setting
==== Abrir caixa de diálogo Selecionar Linha Braille ====
...
%kc:setting
==== Abrir configurações da Apresentação de Objetos ====
...
%kc:setting
==== Abrir configurações de Formatação de Documento ====
...
Por favor consulte a documentação da mesma para obter descrições de onde essas teclas se encontram.
+++ Atribuições de teclas para todos os modelos +++
%kc:beginInclude
|| Nome | Tecla |
...
+++ Atribuições de teclas para Brailliant BI 32, BI 40 e B 80 +++
%kc:beginInclude
|| Nome | Tecla |
...
+++ Atribuições de teclas para Brailliant BI 14 +++
%kc:beginInclude
|| Nome | Tecla |
...
+ Certifique-se de ter instalado o [driver mais recente https://www.papenmeier-rehatechnik.de/en/service/downloadcenter/software/articles/software-braille-devices.html].
+ Abra o Gerenciador de Dispositivos do Windows.
+ Role a lista para baixo até "Controladores USB" ou "Dispositivos USB".
+ Selecione "Dispositivo Papenmeier Braillex USB".
+ Abra as propriedades e mude para a guia "Avançado".
...
+ Desabilite a opção "Carregar VCP".
+
